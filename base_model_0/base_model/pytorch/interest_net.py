...
class InterestNet(nn.Module, EmbeddingMixin):
    ...
    def __init__(self, features, attention_groups, num_classes, embedding_size,
        ...
        total_input_size = 3 * 1 * self.embedding_sizes['item']
        self.mlp = MLP(
            total_input_size,
            hidden_layers,
    ...
    def forward(self, x):
        ...
        for feature in self.features.category_features:
            ...
            embeddings[feature.name + '_bias'] = self.embeddings[
                feature.name + '_bias'](x[feature.name])
        ...
        """-------------cancat all feature ---------------------"""
        # u_emb = torch.cat([embeddings['item'], embeddings['cat']], dim=1)
        u_emb = embeddings['item']
        i_emb = embeddings['group1']
        emb_concat = torch.cat([u_emb, i_emb, u_emb * i_emb], dim=1)
        ...
        output = output + embeddings['item_bias']

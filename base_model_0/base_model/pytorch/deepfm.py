...
class DeepFM(nn.Module, EmbeddingMixin):
    ...
    def __init__(self, features, num_classes, embedding_size, hidden_layers,
        ...
        if use_fm and total_embedding_sizes:
            self.fm = FM(total_embedding_sizes_fm)
            # final_layer_input_size += 1
    ...
    def forward(self, x):
        ...
        final_layer_inputs.append(self.embeddings['item_bias'](x['item']))
        ...
        if embeddings:
            ...
            emb_f1 = torch.cat([embeddings[0], embeddings[1]], dim=2)
            emb_f2 = torch.cat([embeddings[2], embeddings[3]], dim=2)
            emb_concat = torch.cat([emb_f1, emb_f2], dim=1)
            b, f, e = emb_concat.size()
            # fm_2=self.fm(emb_concat)
            # fm
            if self.fm:
                fm = self.fm(emb_concat)
                final_layer_inputs.append(fm)

        # deep
        # emb_concat_deep = torch.cat([emb_f1, emb_f2], dim=1)
        emb_concat_deep = torch.cat(embeddings, dim=2).squeeze(1)
        if self.mlp:
            deep = self.mlp(emb_concat_deep)
            output_deep = self.final_layer(deep)
            # deep1 = deep1 + fm_2
            final_layer_inputs.append(output_deep)

        final_layer_inputs = torch.cat(final_layer_inputs, dim=1)
        output = torch.sum(final_layer_inputs, dim=1).unsqueeze(1)

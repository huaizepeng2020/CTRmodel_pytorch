...
unique_cats_in_hist = Counter(
    itertools.chain(*train_df.hist_cat.apply(lambda x: x.split('\t')).values.tolist()))
...
sequence_features = [
    ...
    Sequence('hist_cat',
             SequenceEncoder(sep="\t", min_cnt=1, max_len=SEQ_MAX_LEN,
                             word2idx=cat_word2idx, idx2word=cat_idx2word),
             embedding_name='cat'),
    Sequence('neg_hist_item',
             SequenceEncoder(sep="\t", min_cnt=1, max_len=SEQ_MAX_LEN,
                             word2idx=mid_word2idx, idx2word=mid_idx2word),
             embedding_name='item'),
    Sequence('neg_hist_cat',
             SequenceEncoder(sep="\t", min_cnt=1, max_len=SEQ_MAX_LEN,
                             word2idx=cat_word2idx, idx2word=cat_idx2word),
             embedding_name='cat')
]
...
augru_attention_groups_with_neg = [
    AttentionGroup(
        name='group1',
        pairs=[{'ad': 'item', 'pos_hist': 'hist_item', 'neg_hist': 'neg_hist_item'},
               {'ad': 'cat', 'pos_hist': 'hist_cat', 'neg_hist': 'neg_hist_cat'}],
        hidden_layers=[80, 40], att_dropout=0.0, gru_type='AUGRU')]
